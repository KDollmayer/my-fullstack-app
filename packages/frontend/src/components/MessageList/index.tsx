...
export default function MessageList() {
  ...
  const waitingMessage = "Waiting for messages...";

  const createMessage = (messageText: string): void => {
    const messageItem: string = messageText;

    axios.post<MessageItem[]>("/messages", { messageItem }).then(async () => {
              ...
              {item.username === userText?.username && (
                <s.UserMessage key={item._id}>
                  <s.DivRight>
                    ...
                    <s.Ptime>{`${item.timeStamp}`}</s.Ptime>
                  </s.DivRight>
                </s.UserMessage>
              )}
              {item.username !== userText?.username && (
    ...
    <>
      <s.MsnMessengerDiv>
        <s.MessengerWindow>
          ...
          <s.messageDiv>{output()}</s.messageDiv>
          ...
          <s.InputDiv>
            ...
            <s.Button onClick={() => createMessage(messageText)}>
              {" "}
              Send{" "}
            </s.Button>
          </s.InputDiv>
